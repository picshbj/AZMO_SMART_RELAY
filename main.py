...
def setRelay(idx, value):
    if 0 <= idx <= 5:
        Relay_Pins[idx] = value
        ...
        os.system('i2cset -y 0 0x1C 0x01 %d' % (cmd))
    
    else:
        print('Set Relay Index Error')
...
def updateRelay():
    ...
    try:
        ...
        for idx, relay in enumerate(RELAYS_PARAM):
            ...
            if result:
                setRelay(idx, 1) # 1 means on
            else:
                setRelay(idx, 0) # 0 means off
...
async def main():
    ...
    readParams()
    os.system('i2cset -y 0 0x1C 0x03 0x00')
    REBOOT_FLAG = False
